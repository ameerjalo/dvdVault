# ByteBooks Digital Boutique

> A production-grade, full-stack e-commerce experience for the discerning reader — a curated storefront, a stateful shopping cart, a personal wishlist, a full order-history flow, and an analytics-driven admin console for end-to-end catalog management.

ByteBooks is a boutique book retailer built as a single deployable web application. It pairs a polished public storefront with a role-gated admin dashboard, all served by a JWT-secured FastAPI backend. The project demonstrates clean architectural separation, defensive security defaults, atomic order processing, data-driven dashboards, and an automated end-to-end smoke suite that runs against any deployment target.
...
- **Storefront** — A hero-led landing page with a curated catalog grid, hover-scale product cards, image lazy-loading, and graceful empty / loading states.
- **Search & filter** — Debounced (200 ms) full-text search (`ilike` on product names) combined with a category selector. Filters compose on the server side for cheap pagination later.
- **Persistent cart** — React Context backed by `localStorage` (`bytebooks_cart`). Cart survives reloads and tab switches; quantities clamp to live stock; duplicate adds merge instead of duplicating rows.
...
- **Order history** — `/orders` page renders collapsible cards for each past purchase. Each card shows order id, formatted date, status pill (completed / pending), grand total, and an expandable detail panel with item thumbnails, quantities, unit prices, and line totals. Backend route: `GET /orders/me`.
- **Wishlist** — Tap the heart icon on any product card to save it for later. Hearts animate from slate to rose, optimistic updates keep the UI snappy, and `/wishlist` renders a dedicated grid of saved books. Backed by a many-to-many `WishlistItem` table on the backend.
- **Toast notifications** — Add-to-cart, wishlist toggles, checkout success, and sign-in all emit non-blocking `react-hot-toast` notifications instead of `window.alert` or inline banners.

### Admin experience

- **Protected dashboard** — `/dashboard` routes are gated by `ProtectedRoute`. Non-admins are redirected to login; admins see the slate-blue console layout.
- **Analytics dashboard** — Three live stat cards (Total Revenue, Orders in the last 7 days, Total Products) feeding off `GET /admin/stats`. Below the stat row sit two `recharts` visualisations:
  - **Recent Sales** — An `AreaChart` plotting daily revenue across a zero-filled 7-day window with a gradient fill.
  - **Stock by Category** — A `BarChart` showing live inventory per category, including an "Uncategorized" bucket when relevant.
- **Full product CRUD** — Create, edit, and delete products via a modal-driven workflow with controlled-input forms. Confirm dialog on delete to prevent accidental removals.
...
- **Image management** — Every product carries an `image_url`. The admin table renders thumbnails with an `onError` fallback so a broken CDN link can never break the UI.
...
| **Frontend** | React 19 · Vite 5 · React Router 7 · Axios · Context API (Auth + Cart) · Recharts (analytics) · react-hot-toast (notifications) · CSS modules with a slate / blue SaaS palette |
...
| `GET` | `/wishlist` | bearer | Products in the current user's wishlist (newest first). |
| `POST` | `/wishlist/{product_id}` | bearer | Toggle a product in the wishlist. Returns `{product_id, added}`. |
...
cd backend
python seed.py            # populates SQLite (ecommerce.db) with 3 categories, 10 products, admin user
uvicorn main:app --reload
```

The seeded admin is `admin@bytebooks.dev` / `AdminPass123!`. **Rotate this credential before any non-local deployment.**
...
| `ALLOWED_ORIGINS` | yes (in prod) | Comma-separated list of allowed CORS origins, e.g. `https://bytebooks.vercel.app`. |
...
| Variable | Required | Description |
| --- | --- | --- |
| `VITE_API_URL` | yes | Public URL of the deployed Railway backend, e.g. `https://bytebooks.up.railway.app`. |
...
6. (Optional) Open a shell on the service and run `python seed.py` to load demo data.
...
4. `vercel.json` ships the SPA rewrite so React Router deep links (e.g. `/dashboard/products`, `/orders`) resolve correctly on refresh.

---

## Testing

End-to-end smoke tests live in `frontend/tests/production.spec.ts` and run against any URL via the `BASE_URL` env var. They validate the two critical paths: the storefront renders products and an admin can sign in and see the Dashboard link in the navbar.
...
```bash
BASE_URL=https://bytebooks.vercel.app \
...
backend/
  ...
  seed.py            Demo data + admin bootstrap
  Procfile           Railway start command (gunicorn + uvicorn workers)
...
frontend/
  src/
    ...
    components/                Modal, ShopCard (with wishlist heart), CartDrawer, PublicNavbar, DashboardLayout, AddProductForm
